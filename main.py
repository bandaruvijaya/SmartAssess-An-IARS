...
try:
    ...
    raise RuntimeError(
        f"Failed to load embedding model '{EMBED_MODEL_PATH}': {e}\n"
        "If you're offline, download the model on a machine with internet and set the "
        "environment variable EMBED_MODEL_PATH to the local model directory. If behind a "
        "proxy, configure HTTP(S)_PROXY/HTTPS_PROXY or ensure DNS/network access to huggingface.co."
    )
